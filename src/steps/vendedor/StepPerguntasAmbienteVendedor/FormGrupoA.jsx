import { useFormContextVendedor } from '../../../context/FormContextVendedor.js'
import { FieldGroup } from '../../../components/FieldGroup/FieldGroup.jsx'
import styles from './StepPerguntasAmbienteVendedor.module.css'

const NAO_SE_APLICA = 'não se aplica'
const OPCOES_FECHAMENTO = [{ valor: 'SIM', label: 'SIM' }, { valor: 'NAO', label: 'NÃO' }]
const OPCOES_CAIXARIA = ['MDP', 'MDF']
const OPCOES_DOBRADICAS = ['Convencionais', 'Amortecimento']
const OPCOES_CORREDICAS = ['Telescópicas', 'Ocultas']

export function FormGrupoA({ instanceId, erros = {} }) {
  const { state, dispatch } = useFormContextVendedor()
  const resp = state.respostasPorAmbiente[instanceId] || {}

  const set = (campo, valor) =>
    dispatch({ type: 'SET_RESPOSTA_AMBIENTE_VENDEDOR', instanceId, campo, valor })

  const campoAltura = (campo, pergunta) => {
    const valor = resp[campo]
    const naoSeAplica = valor === NAO_SE_APLICA
    return (
      <div className={styles.campoAltura}>
        <p className={styles.pergunta}>{pergunta}</p>
        <div className={styles.linhaAltura}>
          <div className={styles.inputComSufixo}>
            <input
              type="number"
              inputMode="numeric"
              min="1"
              value={naoSeAplica ? '' : (valor ?? '')}
              disabled={naoSeAplica}
              onChange={(e) => {
                const raw = e.target.value
                if (raw === '') { set(campo, null); return }
                const n = Number(raw)
                set(campo, Number.isNaN(n) ? null : n)
              }}
            />
            <span className={styles.sufixoMm}>mm</span>
          </div>
          <label className={styles.naoSeAplicaLabel}>
            <input
              type="checkbox"
              checked={naoSeAplica}
              onChange={() => set(campo, naoSeAplica ? null : NAO_SE_APLICA)}
            />
            Não se aplica
          </label>
        </div>
        {erros[campo] && <span className={`${styles.erro} erro-campo`}>{erros[campo]}</span>}
      </div>
    )
  }

  const campoFechado = (campo, pergunta, opcoes) => (
    <div className={styles.campoFechado}>
      <p className={styles.pergunta}>{pergunta}</p>
      <div className={styles.opcoes}>
        {opcoes.map((op) => {
          const valorOp = typeof op === 'string' ? op : op.valor
          const labelOp = typeof op === 'string' ? op : op.label
          return (
            <button
              key={valorOp}
              type="button"
              className={resp[campo] === valorOp ? styles.opcaoAtiva : styles.opcao}
              onClick={() => set(campo, valorOp)}
            >
              {labelOp}
            </button>
          )
        })}
        <button
          type="button"
          className={resp[campo] === NAO_SE_APLICA ? styles.opcaoAtiva : styles.opcao}
          onClick={() => set(campo, NAO_SE_APLICA)}
        >
          Não se aplica
        </button>
      </div>
      {erros[campo] && <span className={`${styles.erro} erro-campo`}>{erros[campo]}</span>}
    </div>
  )

  return (
    <div>
      <FieldGroup titulo="Alturas de instalação">
        {campoAltura('alturaBalcao_mm', 'Qual altura do piso será instalada os balcões?')}
        {campoAltura('alturaArmario_mm', 'Qual altura do piso será instalada os armários?')}
      </FieldGroup>

      <FieldGroup titulo="Acabamento e ferragens">
        {campoFechado('fechamentoTeto', 'Haverá fechamento até o teto?', OPCOES_FECHAMENTO)}
        {campoFechado('caixaria', 'Caixarias ou corpos', OPCOES_CAIXARIA)}
        {campoFechado('dobradicas', 'Dobradiças', OPCOES_DOBRADICAS)}
        {campoFechado('corredicas', 'Corrediças', OPCOES_CORREDICAS)}
      </FieldGroup>

      <FieldGroup titulo="Observações (opcional)">
        <textarea
          className={styles.textarea}
          value={resp.observacoes || ''}
          maxLength={300}
          onChange={(e) => set('observacoes', e.target.value)}
          placeholder="Ex: detalhes adicionais sobre o ambiente..."
          rows={3}
        />
        <span className={styles.contador}>{(resp.observacoes || '').length}/300</span>
      </FieldGroup>
    </div>
  )
}
